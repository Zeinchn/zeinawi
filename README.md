# Тема 2. Базовые операции языка Python  
**ФИО:** Зейн Талеб Шейхна  
**Группа:** ИВТ-23-1  

## Итоговая таблица

| Задание   | Лаб_раб | Сам_раб |
|-----------|:-------:|:-------:|
| Задание 1 |    +    |    +    |
| Задание 2 |    +    |    +    |
| Задание 3 |    +    |    +    |
| Задание 4 |    +    |    +    |
| Задание 5 |    +    |    +    |
| Задание 6 |    +    |    +    |
| Задание 7 |    +    |    +    |
| Задание 8 |    +    |    +    |
| Задание 9 |    +    |    +    |
| Задание 10|    +    |    +    |

Знак "+" – задание выполнено; знак "-" – задание не выполнено.
# Лабораторная работа 2 и Самостоятельная работа 2

## 📘 Лабораторная работа 2

### Задание 1
**Скриншот результата:**  
![lab2_task1](screenshots/lab2_task1.png)  

**Вывод:** Вывел три строки: число, число в виде строки и число с плавающей точкой.

---

### Задание 2
**Скриншот результата:**  
![lab2_task2](screenshots/lab2_task2.png)  

**Вывод:** Получил результат сложения int, float и их комбинации.

---

### Задание 3
**Скриншот результата:**  
![lab2_task3](screenshots/lab2_task3.png)  

**Вывод:** Вывел обычную строку, f-строку с переменной и результат конкатенации строк.

---

### Задание 4
**Скриншот результата:**  
![lab2_task4](screenshots/lab2_task4.png)  

**Вывод:** Преобразовал переменные в bool, int/float и str.

---

### Задание 5
**Скриншот результата:**  
![lab2_task5](screenshots/lab2_task5.png)  

**Вывод:** Ввел три значения через input() и присвоил их переменным.

---

### Задание 6
**Скриншот результата:**  
![lab2_task6](screenshots/lab2_task6.png)  

**Вывод:** Выполнил возведение в степень, деление, целочисленное деление и остаток от деления.

---

### Задание 7
**Скриншот результата:**  
![lab2_task7](screenshots/lab2_task7.png)  

**Вывод:** Умножил строковую переменную на число и получил повторение строки.

---

### Задание 8
**Скриншот результата:**  
![lab2_task8](screenshots/lab2_task8.png)  

**Вывод:** Посчитал количество символов `o` в строке "Hello World".

---

### Задание 9
**Скриншот результата:**  
![lab2_task9](screenshots/lab2_task9.png)  

**Вывод:** Вывел строку "Hello World" в две строки, используя одну строку кода.

---

### Задание 10
**Скриншот результата:**  
![lab2_task10](screenshots/lab2_task10.png)  

**Вывод:** Вывел второй символ строки "Hello World" и слово "Hello".

---

## 📗 Самостоятельная работа 2

### Задание 1
**Скриншот результата:**  
![self2_task1](screenshots/self2_task1.png)  

**Вывод:** Получил False без использования слова `False`.

---

### Задание 2
**Скриншот результата:**  
![self2_task2](screenshots/self2_task2.png)  

**Вывод:** Присвоил значения трём переменным и вывел их в консоль за две строки.

---

### Задание 3
**Скриншот результата:**  
![self2_task3](screenshots/self2_task3.png)  

**Вывод:** Реализовал ввод только целых чисел через `int(input())`.

---

### Задание 4
**Скриншот результата:**  
![self2_task4](screenshots/self2_task4.png)  

**Вывод:** Создал строку ≤ 5 символов и получил строку длиной ≥ 16 символов.

---

### Задание 5
**Скриншот результата:**  
![self2_task5](screenshots/self2_task5.png)  

**Вывод:** Вывел дату в формате "Сегодня день месяц год. Всего хорошего!" с помощью f-строки.

---

### Задание 6
**Скриншот результата:**  
![self2_task6](screenshots/self2_task6.png)  

**Вывод:** Вставил "my" в строку "Hello World" между словами.

---

### Задание 7
**Скриншот результата:**  
![self2_task7](screenshots/self2_task7.png)  

**Вывод:** Определил длину строки "Hello World".

---

### Задание 8
**Скриншот результата:**  
![self2_task8](screenshots/self2_task8.png)  

**Вывод:** Преобразовал строку "HELLO WORLD" в нижний регистр.

---

### Задание 9
**Скриншот результата:**  
![self2_task9](screenshots/self2_task9.png)  

**Вывод:** Решил задачу с числами (например, вычислил квадрат числа).

---

### Задание 10
**Скриншот результата:**  
![self2_task10](screenshots/self2_task10.png)  

**Вывод:** Решил задачу со строками (например, подсчитал количество букв "а" в строке).


## Общий вывод

В ходе выполнения лабораторных и самостоятельных заданий по теме 2 я закрепил базовые навыки работы с языком Python. Я научился:

- использовать переменные и выводить данные на экран;
- работать с вводом данных от пользователя;
- применять арифметические операции и функции для обработки чисел и строк;
- использовать условные операторы и встроенные методы строк;
- создавать простые программы, состоящие из нескольких строк кода.

Выполнение заданий позволило лучше понять синтаксис Python и логику построения программ, а также получить практический опыт, который необходим для дальнейшего изучения языка.
