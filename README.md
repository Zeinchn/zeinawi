# Тема 3. Операторы, условия, циклы  
**ФИО:** Зейн Талеб Шейхна  
**Группа:** ИВТ-23-1  

## Итоговая таблица

| Задание   | Лаб_раб | Сам_раб |
|-----------|:-------:|:-------:|
| Задание 1 |    +    |    +    |
| Задание 2 |    +    |    +    |
| Задание 3 |    +    |    +    |
| Задание 4 |    +    |    +    |
| Задание 5 |    +    |    +    |
| Задание 6 |    +    |    -    |
| Задание 7 |    +    |    -    |
| Задание 8 |    +    |    -    |
| Задание 9 |    +    |    -    |
| Задание 10|    +    |    -    |

Знак "+" – задание выполнено; знак "-" – задание не выполнено.  

# Лабораторная работа 3 и Самостоятельная работа 3

## 📘 Лабораторная работа 3

### Задание 1
**Скриншот результата:**  
![lab3_task1](screenshots/lab3_task1.png)  

**Вывод:** Сравнил две переменные, ввёл через консоль, вывел результат выполнения условия.

---

### Задание 2
**Скриншот результата:**  
![lab3_task2](screenshots/lab3_task2.png)  

**Вывод:** Определил диапазон значения переменной (меньше 0, от 0 до 10, больше 10).

---

### Задание 3
**Скриншот результата:**  
![lab3_task3](screenshots/lab3_task3.png)  

**Вывод:** Проверил наличие переменной в массиве с помощью оператора in.

---

### Задание 4
**Скриншот результата:**  
![lab3_task4](screenshots/lab3_task4.png)  

**Вывод:** Если значение есть в массиве, программа определила, чётное оно или нечётное.

---

### Задание 5
**Скриншот результата:**  
![lab3_task5](screenshots/lab3_task5.png)  

**Вывод:** С помощью цикла or перебрал значения от 0 до 10 и протестировал разные сравнения.

---

### Задание 6
**Скриншот результата:**  
![lab3_task6](screenshots/lab3_task6.png)  

**Вывод:** Реализовал поиск значения в строке через цикл or и оператор lse.

---

### Задание 7
**Скриншот результата:**  
![lab3_task7](screenshots/lab3_task7.png)  

**Вывод:** Реализовал цикл в обратном порядке (от 10 до 0) и применил своё вычисление.

---

### Задание 8
**Скриншот результата:**  
![lab3_task8](screenshots/lab3_task8.png)  

**Вывод:** Написал цикл while с условием, чтобы избежать бесконечного выполнения.

---

### Задание 9
**Скриншот результата:**  
![lab3_task9](screenshots/lab3_task9.png)  

**Вывод:** Использовал вложенные циклы с разными итерируемыми переменными и проверкой внутри.

---

### Задание 10
**Скриншот результата:**  
![lab3_task10](screenshots/lab3_task10.png)  

**Вывод:** Применил lag для проверки наличия нечётного числа в массиве.

---

## 📗 Самостоятельная работа 3

### Задание 1
**Скриншот результата:**  
![self3_task1](screenshots/self3_task1.png)  

**Вывод:** С помощью цикла or, операций *=5 и +=1 преобразовал число 1 в 31.

---

### Задание 2
**Скриншот результата:**  
![self3_task2](screenshots/self3_task2.png)  

**Вывод:** Вывел строку "Hello World" в обратном порядке, каждая буква с новой строки, ≤3 строк кода.

---

### Задание 3
**Скриншот результата:**  
![self3_task3](screenshots/self3_task3.png)  

**Вывод:** Программа принимает число от 0 до 10 и определяет, в каком диапазоне оно находится.

---

### Задание 4
**Скриншот результата:**  
![self3_task4](screenshots/self3_task4.png)  

**Вывод:** Выполнил операции со строкой: длина, lowercase, подсчёт гласных, замена "ugly" → "beauty", проверка начала/конца.

---

### Задание 5
**Скриншот результата:**  
![self3_task5](screenshots/self3_task5.png)  

**Вывод:** Составил программу из предложенных фрагментов кода и получил нужный вывод.

---

## Общий вывод

В ходе выполнения лабораторных и самостоятельных заданий по теме 3 я закрепил навыки работы с операторами, условиями и циклами в Python. Я научился:

- сравнивать значения и работать с логическими условиями;
- использовать циклы or и while, включая вложенные и обратные варианты;
- применять оператор in, lse для циклов и флаговые переменные;
- обрабатывать строки: менять регистр, заменять слова, считать символы;
- контролировать диапазоны чисел и корректность пользовательского ввода.

Эти задания позволили глубже понять использование условий и циклов в Python и подготовили к более сложным практическим задачам.
